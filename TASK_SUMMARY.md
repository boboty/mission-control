...
- 状态和优先级映射为中文显示
- 使用 Blob + download 属性下载
- 自动生成带日期的文件名

#### 2. PDF 导出
- 使用浏览器打印功能（window.print()）
- 生成打印友好的 HTML 页面
- 包含样式化的表格和状态徽章
- 支持 A4 纸张格式
- 自动打开打印对话框
...
#### 3. UI 集成
- 在任务看板/管线视图顶部添加"导出"按钮
- 点击弹出菜单：导出 CSV / 导出 PDF
- 导出时显示加载状态（⏳ 动画）
- 尊重当前筛选/搜索条件
- 点击外部关闭菜单

### 修改文件
- `src/lib/export-utils.ts` - 新增导出工具函数（7.2KB）
- `src/components/dashboard/TaskBoard.tsx` - 添加导出按钮和功能
- `src/components/dashboard/PipelineList.tsx` - 添加导出按钮和功能

### 验收标准
- [x] 导出按钮位于筛选栏顶部
- [x] CSV 导出包含所有指定字段
- [x] CSV 支持中文（UTF-8 BOM）
- [x] PDF 导出使用浏览器打印功能
- [x] 导出时显示加载状态
- [x] 导出尊重当前筛选条件
- [x] UI 风格与现有设计一致
...
### 详细说明
见 `TASK_95_EXPORT_FEATURE_REPORT.md`
